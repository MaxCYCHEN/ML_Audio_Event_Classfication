...
def gen_h_user_file(config):
    ...
    with open(os.path.join(path, "audio_det_model_config.h"), "wt") as f:
        f.write('/**')
        f.write('******************************************************************************\n')
        f.write('* @file    audio_det_model_config.h\n')
        f.write('* @author  \n')
        ...
        f.write('#endif\n')
        f.write('\n')
        f.write('#define AUDIO_DET_MODE_NB_OUTPUT            (1U)\n')
        f.write('#define AUDIO_DET_MODE_OUTPUT_1             (CTRL_AI_CLASS_DISTRIBUTION)\n')
        f.write('#define AUDIO_DET_MODE_CLASS_NUMBER         ({}U)\n'.format(n_classes))
        f.write('#define AUDIO_DET_MODE_CLASS_LIST           {}\n'.format(classes))
        ...
        f.write('#define AUDIO_DET_PREPROC                   (CTRL_AI_SPECTROGRAM_LOG_MEL)\n')
        f.write('#define AUDIO_DET_SPECTROGRAM_NMEL          ({}U)\n'.format(config.feature_extraction.n_mels))
        f.write('#define AUDIO_DET_SPECTROGRAM_COL           ({}U)\n'.format(config.feature_extraction.patch_length))
        f.write('#define AUDIO_DET_SPECTROGRAM_HOP_LENGTH    ({}U)\n'.format(config.feature_extraction.hop_length))
        f.write('#define AUDIO_DET_SPECTROGRAM_NFFT          ({}U)\n'.format(config.feature_extraction.n_fft))
        f.write('#define AUDIO_DET_SPECTROGRAM_WINDOW_LENGTH ({}U)\n'.format(config.feature_extraction.window_length))
        if config.feature_extraction.norm is None or config.feature_extraction.norm == "None":
                f.write('#define AUDIO_DET_SPECTROGRAM_NORMALIZE     (0U) // (1U)\n')
        elif config.feature_extraction.norm == "slaney":
            f.write('#define AUDIO_DET_SPECTROGRAM_NORMALIZE     (1U) // (0U)\n')
        else:
        ...
        if config.feature_extraction.htk:
            f.write('#define AUDIO_DET_SPECTROGRAM_FORMULA       (MEL_HTK) //MEL_SLANEY\n')
        else:
            f.write('#define AUDIO_DET_SPECTROGRAM_FORMULA       (MEL_SLANEY) //MEL_HTK\n')
        f.write('#define AUDIO_DET_SPECTROGRAM_FMIN          ({}U)\n'.format(config.feature_extraction.fmin))
        f.write('#define AUDIO_DET_SPECTROGRAM_FMAX          ({}U)\n'.format(config.feature_extraction.fmax))
        if int(config.feature_extraction.power) == 1:
            f.write('#define AUDIO_DET_SPECTROGRAM_TYPE          (SPECTRUM_TYPE_MAGNITUDE) // ,  /*!< magnitude spectrum */\n')
        elif int(config.feature_extraction.power) == 2:
            f.write('#define AUDIO_DET_SPECTROGRAM_TYPE          (SPECTRUM_TYPE_POWER) // ,  /*!< squared magnitude spectrum */\n')
        else:
            raise ValueError("Higher powers than 2 for spectrogram not supported in Getting Started.")
        if config.feature_extraction.to_db:
            f.write('#define AUDIO_DET_SPECTROGRAM_LOG_FORMULA   (LOGMELSPECTROGRAM_SCALE_DB) //LOGMELSPECTROGRAM_SCALE_LOG\n')
        else:
            f.write('#define AUDIO_DET_SPECTROGRAM_LOG_FORMULA   (LOGMELSPECTROGRAM_SCALE_LOG) //LOGMELSPECTROGRAM_SCALE_DB\n')
        # NOTE : Make this a deployment parameter
        # For now, just disable it if spec is db scale
        if config.feature_extraction.to_db:
            f.write('#define AUDIO_DET_SPECTROGRAM_SILENCE_THR   (0) // 0 means disabled\n')
        else:
            f.write('#define AUDIO_DET_SPECTROGRAM_SILENCE_THR   (100) // 0 means disabled\n')
        f.write('#define AUDIO_DET_SPECTROGRAM_WIN           (userWin)\n')
        f.write('#define AUDIO_DET_SPECTROGRAM_MEL_LUT       (user_melFilterLut)\n')
        f.write('#define AUDIO_DET_SPECTROGRAM_MEL_START_IDX (user_melFiltersStartIndices)\n')
        f.write('#define AUDIO_DET_SPECTROGRAM_MEL_STOP_IDX  (user_melFiltersStopIndices)\n')
        f.write('#define AUDIO_DET_OOD_THR                   ({}F)\n'.format(config.model.unknown_class_threshold))
